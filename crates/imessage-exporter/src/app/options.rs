...
/// Default export format for this fork (`-f` / `--format`).
pub const DEFAULT_EXPORT_FORMAT: &str = "json";
/// Default backup platform (`-a` / `--platform`).
pub const DEFAULT_PLATFORM: &str = "iOS";
/// Default iOS backup root when `-p` / `--db-path` is omitted (`-p` / `--db-path`).
pub const DEFAULT_IOS_BACKUP_PATH: &str = "staging/imessage/ios_backup";
...
pub const ABOUT: &str = concat!(
    "The `imessage-exporter-json` binary exports iMessage data to\n",
    "`txt`, `html`, or `json` formats. It can also run diagnostics\n",
...
impl Options {
    pub fn from_args(args: &ArgMatches) -> Result<Self, RuntimeError> {
        ...
        // Build the export type. Default to JSON unless running diagnostics only.
        ...
        let db_path = match user_path {
            ...
            None => PathBuf::from(DEFAULT_IOS_BACKUP_PATH),
        };
...
fn get_command() -> Command {
    Command::new("iMessage Exporter (JSON)")
...
mod arg_tests {
    ...
    use crate::app::{
        compatibility::attachment_manager::{AttachmentManager, AttachmentManagerMode},
        export_type::ExportType,
        options::{Options, DEFAULT_IOS_BACKUP_PATH, get_command, validate_path},
    ...
    fn can_build_option_diagnostic_flag() {
        ...
        let expected = Options {
            db_path: PathBuf::from(DEFAULT_IOS_BACKUP_PATH),
            attachment_root: None,
    ...
    #[test]
    fn can_build_option_attachment_manager_with_default_export() {
        let command = get_command();
        let args = command.get_matches_from(["imessage-exporter", "-c", "clone"]);
        let actual = Options::from_args(&args).unwrap();
        assert_eq!(actual.export_type, Some(ExportType::Json));
        assert_eq!(actual.platform, Platform::iOS);
    }

    #[test]
    fn can_build_option_export_path_with_default_export() {
        let command = get_command();
    ...
    #[test]
    fn can_build_option_start_date_with_default_export() {
        let command = get_command();
    ...
    #[test]
    fn can_build_option_end_date_with_default_export() {
        let command = get_command();
    ...
    #[test]
    fn can_build_option_caller_id_with_default_export() {
        let command = get_command();
    ...
    #[test]
    fn can_build_option_custom_name_with_default_export() {
        let command = get_command();
    ...
    #[test]
    fn can_build_option_contact_filter_with_default_export() {
        let command = get_command();
    ...
    #[test]
    fn can_build_option_no_lazy_with_default_export() {
    ...
    #[test]
    fn can_build_option_no_progress_with_default_export() {
