...
pub trait MessageWriter<'a>: MessageFormatter<'a> {
    ...
    /// Optional per-conversation header, written immediately after
    /// [`write_file_header`](Self::write_file_header) when a chat file is first
    /// created. Unlike `write_file_header`, this hook has access to the resolved
    /// [`Chat`] so the output can include conversation-level metadata
    /// (participants, title, type). The string is computed before the
    /// [`ExportState`] is borrowed mutably to avoid aliasing conflicts.
    /// Returning `None` (the default) emits nothing. Not written for the
    /// orphaned file, which has no conversation.
    fn conversation_header(&self, _chatroom: &Chat) -> Option<String> {
        None
    }
}
    ...
    W: MessageWriter<'a>,
{
    ...
    let Some((chatroom, _)) = config.conversation(message) else {
        return Ok(&mut writer.state_mut().orphaned);
    };
    ...
    let filename = {
        ...
        match state.route.get(&chatroom_rowid) {
    ...
    // Compute the optional per-conversation header only when the chat file has
    // not been opened yet. Done before re-borrowing state mutably, since
    // `conversation_header` borrows the writer.
    let conversation_header = if writer.state().files.contains_key(&filename) {
        None
    } else {
        writer.conversation_header(chatroom)
    };
    ...
    match state.files.entry(filename) {
        ...
        Vacant(entry) => {
            ...
            if !file_exists {
                ...
                if let Some(header) = conversation_header {
