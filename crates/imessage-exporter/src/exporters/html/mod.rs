...
impl<'a> MessageFormatter<'a> for HTML<'a> {
    fn format_attachment(
        ...
        if let Err(render) = prepare_attachment(self.config, &self.state, attachment, message, false) {
            return render;
        }
...
impl<'a> HTML<'a> {
    ...
    fn format_sticker_inline(&self, sticker: &mut Attachment, message: &Message) -> String {
        let (embed_path, label) =
            match prepare_attachment(self.config, &self.state, sticker, message, false) {
                Ok(()) => {
